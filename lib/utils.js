...
/**
 * Converts an application definition, given a name of an application,
 * and the environment's manifest, into an object that the
 * aws-sdk can use directly.
 *
...
function getEnvConfig (appName, manifest) {
  var result = {
    ApplicationName: appName,
    EnvironmentName: manifest.name,
    CNAMEPrefix: manifest.CNAMEPrefix,
    Description: manfiest.description,
    Tier: { Name: manfiest.tier },
    Tags: Object.keys(manifest.tags || {}).map(function (key) { return { Key: key, Value: manifest.tags[key] }; }),
    SolutionStackName: manifest.solutionStackName
  };

  result.OptionSettings = createOptionSettings(manifest);
...
function getEnvironmentsInfo (eb, params, filter) {
  return eb.describeEnvironments(params || {}).then(function (envs) {
    envs = envs || [];
...
function swapCNAMEs (eb, envName1, envName2) {
  return eb.swapEnvironmentCNAMEs({
    DestinationEnvironmentName: envName2,
    SourceEnvironmentName: envName1
  });
}
exports.swapCNAMEs = swapCNAMEs;

function destroyEnvironment (eb, envName) {
  return eb.terminateEnvironment({ EnvironmentName: envName, TerminateResources: true });
}
