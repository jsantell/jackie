...
function Jackie (config) {
  ...
  this._eb = config.mock ? new ElasticBeanstalkMock(this.config) : new ElasticBeanstalk(this.config);
}
...
Jackie.prototype.createApplication = function createApplication (definition) {
  var application = new Application(this._eb, definition);
...
Jackie.prototype.getApplications = function getApplications () {
  ...
  return this._eb.describeApplications().then(function (desc) {
    return when.all(desc.Applications.map(function (app) {
      return new Application(jackie._eb, { name: app.ApplicationName, description: app.Description });
    }));
  });
};
