...
function Environment (eb, appName, envName, description) {
  this._eb = eb;
...
Environment.prototype.info = function info (options) {
  var env = this;
  ...
  return this._eb.describeEnvironments({ EnvironmentNames: [this.envName], ApplicationName: this.appName }).then(function (data) {
    var data = data.Environments.length ? data.Environments[0] : null;
    env._cachedDescription = data;
    return data;
  });
};
