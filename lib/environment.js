...
/**
 * Creates and/or updates the environment based off the `manifest`. If environment already exists,
 * update it based off of the `manifest` values (same as `update` method). Otherwise, create the
 * environment with the config in `manifest`.
 *
 * @param {Object} manifest
 * @return {Promise->Environment}
 */

Environment.prototype.initialize = function initialize (manifest) {
  var env = this;

  var params = _.extend({}, utils.getEnvConfig(env.appName, manifeset), {
    EnvironmentName: this.envName
  });

  this.info().then(function (data) {
    // If environment doesn't exist, create it
    if (!data) {
      return env._eb.createEnvironment(params);
    }
    // Otherwise, update everything in the manifest
    else {
      return env._eb.updateEnvironment(params);
    }
  }).then(function () {
    return env;
  });
...
Environment.prototype._createOnAWS = function createOnAWS (manifest, version) {
